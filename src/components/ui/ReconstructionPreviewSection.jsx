...
const PreviewSvg = ({ title, points, anchor, surface, normal, projector, embedded = false }) => {
  if (!points.length && !(surface.mesh || []).length) {
    return (
      <div
        className={
          embedded
            ? 'px-1 py-3 text-center text-[10px] text-gray-400'
            : 'rounded border border-gray-800 bg-gray-950 px-2 py-3 text-center text-[10px] text-gray-400'
        }
      >
        {title}: no points
      </div>
  ...
  const meshPoints = (surface.mesh || []).map((point) => ({
    ...point,
    id: `mesh:${point.id}`,
    meshPoint: true,
  }));
  ...
  const pointsWithAnchor = anchor
    ? [...meshPoints, ...points, { ...anchor, id: 'anchor' }]
    : [...meshPoints, ...points];
  ...
  const landmarkPoints = projectedPoints.filter(
    (point) => point.id !== 'anchor' && !String(point.id).startsWith('mesh:'),
  );
  ...
  return (
    <div className={embedded ? 'px-0 py-1' : 'rounded border border-gray-800 bg-gray-950 px-2 py-2'}>
      ...
      >
        <title>{title} reconstruction preview</title>
...
export const ReconstructionPreviewSection = ({ details, embedded = false }) => {
  ...
  const geometricConsistency =
    details.reconstructionGeometricConsistency ?? preview?.statistics?.geometricConsistency;
    ...
    <div className={embedded ? 'space-y-2' : 'mt-3 rounded border border-gray-800 bg-gray-950/70 p-2'}>
      {!embedded && (
        <div className="mb-2 flex items-center justify-between gap-2">
          <span className="text-[10px] uppercase tracking-wide text-gray-400">3D Reconstruction</span>
          <span
      ...
      <div className="mt-2 text-[10px]">
        <DiagnosticRow
          label="Inferred position"
          value={formatPoint2(position)}
          tone={position ? 'good' : 'warn'}
        />
        <DiagnosticRow
          label="Inferred normal"
          value={formatVector3(normal)}
          tone={normal ? 'good' : 'warn'}
        />
        <DiagnosticRow
          label="Inferred scale"
          value={formatNumber(planar?.scale, 2)}
          tone={planar?.scale ? 'good' : 'warn'}
        />
        <DiagnosticRow
          label="Inferred roll"
          value={formatDegrees(planar?.rotation)}
          tone={typeof planar?.rotation === 'number' ? 'good' : 'warn'}
        />
        <DiagnosticRow
          label="Target class"
          value={details.targetClass || 'N/A'}
          tone={details.targetClass ? 'good' : 'warn'}
        />
        <DiagnosticRow
          label="Surface model"
          value={previewSurface.model || 'N/A'}
          tone={previewSurface.model ? 'good' : 'warn'}
        />
        <DiagnosticRow
          label="Depth model"
          value={details.reconstructionDepthStatus || 'N/A'}
          tone={details.reconstructionDepthStatus === 'ready' ? 'good' : 'warn'}
        />
        <DiagnosticRow label="Depth provider" value={details.reconstructionDepthProvider || 'N/A'} />
        <DiagnosticRow
          label="Depth inference"
          value={`${formatNumber(details.reconstructionDepthInferenceTime, 1)} ms`}
        />
        <DiagnosticRow
          label="Preview points"
          value={`${previewPoints.length}/${totalLandmarks}`}
          tone={previewPoints.length >= 18 ? 'good' : 'warn'}
        />
        <DiagnosticRow
          label="Map confidence"
          value={formatPercent(mapConfidence)}
          tone={(mapConfidence ?? 0) > 0.55 ? 'good' : 'warn'}
        />
        <DiagnosticRow
          label="Avg support"
          value={formatPercent(averageSupport)}
          tone={(averageSupport ?? 0) > 0.65 ? 'good' : 'warn'}
        />
        <DiagnosticRow
          label="Geometry consistency"
          value={formatPercent(geometricConsistency)}
          tone={(geometricConsistency ?? 0) > 0.62 ? 'good' : 'warn'}
        />
        <DiagnosticRow
          label="Mature points"
          value={matureLandmarks}
          tone={matureLandmarks >= 18 ? 'good' : 'warn'}
        />
        <DiagnosticRow
          label="Surface edges"
          value={previewSurface.edges.length}
          tone={previewSurface.edges.length >= 18 ? 'good' : 'warn'}
        />
        <DiagnosticRow
          label="Surface faces"
          value={previewSurface.faces.length}
          tone={previewSurface.faces.length >= 12 ? 'good' : 'warn'}
        />
      </div>
    </div>
  );
