...
export const renderAnchorOverlay = (ctx, params) => {
  ctx.save();
  renderAnchorMode(ctx, params);
  ctx.restore();
};

function renderAnchorMode(ctx, { anchor, anchorState, trackedPoints = [], showObjectSupport = false }) {
...
const objectRegionForAnchor = (anchor, metrics) => {
  return (
    regionFromBounds(metrics.currentObjectSupportMaskBounds) ||
    regionFromBounds(metrics.objectSupportMaskBounds) ||
    regionFromBounds(anchor.selectionRegion?.objectSupportMask?.bbox) ||
    regionFromBounds(anchor.selectionRegion)
  );
};
...
const drawObjectMaskPreview = (ctx, maskPreview) => {
  ...
  ctx.save();
  ctx.strokeStyle =
    maskPreview.source === 'tap-local' ? 'rgba(250, 204, 21, 0.76)' : 'rgba(45, 212, 191, 0.92)';
  ctx.fillStyle = maskPreview.source === 'tap-local' ? 'rgba(250, 204, 21, 0.2)' : 'rgba(20, 184, 166, 0.22)';
  ctx.lineWidth = 1.5;
  ctx.setLineDash([5, 4]);
...
function drawSelectedObjectRegion(ctx, anchor, metrics) {
  ...
  ctx.save();
  ctx.strokeStyle =
    metrics.currentObjectSupportMaskBounds || anchor.selectionRegion?.objectSupportMask
...
function drawLiveReconstruction(ctx, reconstructionPreview) {
  ...
  for (const face of faces.slice(0, 48)) {
    ...
    ctx.fillStyle = face.reliability >= 0.68 ? 'rgba(20, 184, 166, 0.15)' : 'rgba(15, 23, 42, 0.18)';
  ...
  for (const edge of edges.slice(0, 96)) {
    ...
    ctx.strokeStyle = edge.reliability >= 0.7 ? 'rgba(34, 197, 94, 0.72)' : 'rgba(148, 163, 184, 0.48)';
...
function drawTrackedLandmarks(ctx, trackedPoints, reconstructionPreview) {
...
function drawSurfaceNormal(ctx, anchorState, centerX, centerY) {
...
const readinessProgress = (metrics) => {
  ...
  const activeLandmarks =
    metrics.activeLandmarkCount ?? metrics.activeLandmarks ?? metrics.keypointCount ?? 0;
  return clamp(Math.max(objectOwned, activeLandmarks) / 32, 0, 1);
};

function drawReadinessRing(ctx, centerX, centerY, metrics, radius) {
...
function drawAnchorInfo(ctx, anchor, anchorState, centerX, centerY) {
...
export const renderDebugStats = (ctx, { fps, cameraFrameCost, processingTime, objectCount }) => {
  ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
  ctx.fillRect(10, 10, 200, 60);
  ctx.fillStyle = '#00ff00';
  ctx.font = '12px monospace';
  ctx.fillText(`FPS: ${fps.toFixed(1)}`, 15, 25);
  ctx.fillText(`Camera: ${cameraFrameCost.toFixed(1)}ms`, 15, 40);
  ctx.fillText(`Tracking: ${processingTime.toFixed(1)}ms`, 15, 55);
 ...
 * @param {Object} boundingBox - Optional selected-object bounding box
