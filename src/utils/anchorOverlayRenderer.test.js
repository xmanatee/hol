...
import { renderAnchorOverlay } from './anchorOverlayRenderer.js';
...
const createContext = () => {
  ...
  return {
    ...
    set globalAlpha(value) {
      calls.push({ method: 'globalAlpha', args: [value] });
    },
    set font(value) {
      calls.push({ method: 'font', args: [value] });
    },
  };
};
...
test('anchor overlay draws object bounds live reconstruction and landmarks', () => {
  ...
  assert.ok(
    ctx.calls.some((call) => call.method === 'strokeRect'),
    'draws selected-object support bounds',
  );
  assert.ok(
    ctx.calls.filter((call) => call.method === 'arc').length >= 5,
    'draws anchor, progress ring, and landmark points',
  );
  assert.ok(
    ctx.calls.filter((call) => call.method === 'lineTo').length >= 6,
    'draws reconstruction geometry and normal/crosshair lines',
  );
  assert.ok(
    ctx.calls.some((call) => call.method === 'fillText' && /Map: 4f 3p 46%/.test(call.args[0])),
    'summarizes reconstruction progress',
  );
});
...
test('candidate anchor still visualizes sparse bootstrap landmarks without reconstruction preview', () => {
  ...
  renderAnchorOverlay(ctx, {
    anchor: {
      position: { x: 80, y: 90 },
      selectionRegion: { x1: 30, y1: 40, x2: 140, y2: 160 },
    },
    anchorState: {
      state: 'candidate',
      metrics: {
  ...
  assert.ok(
    ctx.calls.some((call) => call.method === 'strokeRect'),
    'draws selection support bounds',
  );
  assert.ok(
    ctx.calls.filter((call) => call.method === 'arc').length >= 5,
    'draws sparse landmarks plus anchor and progress',
  );
  assert.ok(
    ctx.calls.some((call) => call.method === 'fillText' && /Face: building/.test(call.args[0])),
    'keeps face readiness visible',
  );
});
...
test('anchor overlay draws object mask preview points instead of filling the whole mask bbox', () => {
  ...
  renderAnchorOverlay(ctx, {
    anchor: {
      position: { x: 170, y: 155 },
      selectionRegion: { x1: 90, y1: 80, x2: 280, y2: 260 },
    },
    anchorState: {
      state: 'mapping',
      metrics: {
  ...
  assert.equal(
    ctx.calls.some(
      (call) =>
        call.method === 'fillRect' && call.args[0] === 104 && call.args[1] === 92 && call.args[2] === 138,
    ),
    false,
...
test('normal anchor overlay hides object support bounds outside canvas debug mode', () => {
  ...
  assert.ok(
    ctx.calls.some((call) => call.method === 'arc'),
    'still draws the anchor marker',
  );
});
