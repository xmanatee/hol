...
export class DirectPhotometricReconstructor {
  ...
  updateReferenceRegion(templateRegion, targetClass = this.targetClass) {
    const previousSurfaceModel = this.surfaceModel;
    this.templateRegion = { ...templateRegion };
    this.targetClass = targetClass;
    this.surfaceModel = modelFromRegion(templateRegion, targetClass);
    if (this.surfaceModel !== previousSurfaceModel) {
      this.frameConsensusCache = null;
      this.referenceFitCache = null;
    }
  }
  ...
  addFrameFromTrackedPoints(trackedPoints, timestamp = performance.now(), grayImage = null, options = {}) {
    ...
    this.state =
      this.frames.length >= this.minFrames && statistics.mapConfidence >= 0.45 ? 'ready' : 'mapping';
    this.lastFailureReason = this.state === 'ready' ? null : 'Move object through more photometric views';
    return this._createState(statistics, options);
  }
  ...
  estimatePoseFromTrackedPoints(trackedPoints, grayImage = null, { includePreview = true } = {}) {
    ...
    const rawObservations = this._photometricObservationsFromTrackedPoints(trackedPoints, grayImage).filter(
      (observation) => this._isUsableObservation(observation),
    );
    const consensus = this._consensusOptions();
    const consensusOptions = {
      minInliers: this.minSurfels,
    ...
    const fit = this._fitAttachmentTransform(observations, fitOptions);
    ...
    const confidence = fit.recoveryKind ? Math.min(fit.confidence, 0.72) : fit.confidence;
    const result = {
      success: true,
      method: DIRECT_PHOTOMETRIC_POSE_MODEL,
  ...
  _recordSurfels(observations) {
    observations.forEach((observation) => {
      ...
      const descriptor = previous
        ? previous.descriptor.map(
            (value, index) => value + (observation.photometric.values[index] - value) / observationsCount,
          )
      ...
      this.surfels.set(observation.id, {
        ...
        gradient:
          (previous?.gradient || 0) +
          (observation.photometric.gradient - (previous?.gradient || 0)) / observationsCount,
        residualMean:
          (previous?.residualMean || 0) + (residual - (previous?.residualMean || 0)) / observationsCount,
  ...
  _isUsableObservation(observation) {
    ...
    return surfel.observations >= Math.max(3, this.minFrames - 2) && surfel.gradient >= 6;
  }
  ...
  _referenceFit() {
    ...
    this.referenceFitCache = { fit: referenceFit };
    return referenceFit;
  }
  ...
  _statistics() {
    ...
    const averageSupport =
      surfels.reduce((sum, item) => sum + item.observations / Math.max(this.frames.length, 1), 0) /
      surfels.length;
    const averageReliability =
      surfels.reduce((sum, item) => {
    ...
    const matureLandmarks = surfels.filter(
      (item) => item.observations >= this.minFrames && item.gradient >= 6,
    ).length;
